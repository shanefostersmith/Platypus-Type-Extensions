...
def test_categories(category_type):
    parent_sol, offspring_sol = create_one_var_solutions(category_type)
...
def test_step_range(step_value_type):
    parent_sol, offspring_sol = create_one_var_solutions(step_value_type)
...
def test_multi_real_crossover(multi_real_with_crossover):
    ...
    parent_sol, offspring_sol = create_one_var_solutions(multi_real_with_crossover, nparents, noffspring)
    orig_parent_categories = [sol.variables[0] for sol in parent_sol]
    copy_indices = [0] if noffspring == 1 else [None, 1]
...
def test_multi_real_mutation(multi_real_with_mutation):
    _, offspring_sol = create_one_var_solutions(multi_real_with_mutation)
...
def test_real_list_crossover(real_list_with_crossover):
    ...
    parent_sol, offspring_sol = create_one_var_solutions(real_list_with_crossover, nparents, noffspring)
...
def test_real_list_mutation(real_list_with_mutation):
    _, offspring_sol = create_one_var_solutions(real_list_with_mutation)
...
def test_multi_int_crossover(multi_int_with_crossover):
    parent_sol, offspring_sol = create_one_var_solutions(multi_int_with_crossover, nparents = 3, noffspring = 2)
    orig_parent_vars = [sol.variables[0] for sol in parent_sol]
